...
import 'package:typesafe_firebase_core/exception/exceptions.dart';
...
abstract class BaseModel {
  ...
  static final HashMap<Type, Converter<dynamic>> _converter = HashMap<Type, Converter<dynamic>>();
  ...
  /// );
  /// ```
  static void register<T extends BaseModel>({required ToJson<T> toJson, required FromJson<T> fromJson}) {
    _converter[T] = (
  ...
  ///
  /// Throws an [TypeNotRegisteredException] if the type [T] has not been registered with [BaseModel].
  static Converter<T> getConverter<T>() {
    final c = _converter[T];

    return c == null
        ? throw TypeNotRegisteredException(T.toString())
        : (toJson: (T item) => c.toJson(item), fromJson: (Map<String, dynamic> json) => c.fromJson(json) as T);
  }
}
